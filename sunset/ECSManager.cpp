...
void ECSManager::SystemPhysicsUpdate(float dt, WorldChanger& worldChanger) {
    // Ray casting
    ...
    for (const auto& body : bodies) {
        ...
        worldChanger.PushPositionChange(positionChange);
    }
    ...
    for (const auto& raycastCollision : raycastCollisions) {
        ...
        if (raycastCollision.lengthSquaredBeforeCollision <= checkValue && raycastCollision.lengthSquaredBeforeCollision < minDistance) {
            ...
            vector<PositionChange>& positionChanges = worldChanger.GetPositionChanges();
...
WorldState ECSManager::GetCurrentWorldState() const {
